...
uint8_t TM1638lite::readButtons()
{
  ...
  for (uint8_t i = 0; i < 4; i++)
  {
    buttons |= shiftIn(DATA_IO, CLOCK_IO, LSBFIRST) << i;
  }
  pinMode(DATA_IO, OUTPUT);
  digitalWrite(STROBE_IO, HIGH);
...
//добавил необязательный параметр  - рисование точки вместе с числом
void TM1638lite::displaySS(uint8_t position, uint8_t value, boolean dot = false) { 
  sendCommand(WRITE_LOC);
  digitalWrite(STROBE_IO, LOW);
  shiftOut(DATA_IO, CLOCK_IO, LSBFIRST, 0xC0 + (position << 1));
  shiftOut(DATA_IO, CLOCK_IO, LSBFIRST, value | (dot ? 0b10000000 : 0));
  digitalWrite(STROBE_IO, HIGH);
}

void TM1638lite::displaySS(uint8_t position, uint8_t value) { 
  sendCommand(WRITE_LOC);
  digitalWrite(STROBE_IO, LOW);
  shiftOut(DATA_IO, CLOCK_IO, LSBFIRST, 0xC0 + (position << 1));
  shiftOut(DATA_IO, CLOCK_IO, LSBFIRST, value);
...
void TM1638lite::displayASCII(uint8_t position, uint8_t ascii) {
  boolean dot = false;
  displaySS(position, ss[ascii], dot);
}                                                                      

void TM1638lite::displayASCII(uint8_t position, uint8_t ascii, boolean dot) {
  displaySS(position, ss[ascii], dot);
}                                                                      
